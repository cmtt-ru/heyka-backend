...
module.exports = class WorkspaceService extends Schmervice.Service {
  ...
  async createChannel(workspaceId, userId, channel) {
    ...
    if (!channelInfo.is_private) {
      ...
      const creatorConnections = await connectionService.getUserConnections(userId);
      this.server.apiEvents.emit('subscribe-sockets', {
        room: eventNames.rooms.channelRoom(channelInfo.id),
  ...
  async deleteChannel(channelId) {
    ...
    if (messages.length) {
      ...
      for (let i in messages) {
        ...
        // collect endpoints which should be notified about invite cancelled
        if (messages[i].push_endpoints && messages[i].push_endpoints.length > 0) {
          const pushesToSend = messages[i].push_endpoints.map(
            endpoint => notificationService.sendPushNotificationToDevice(
              endpoint,
              {
          ...
          sendPushPromises.push(...pushesToSend);
        }
      }
    }
  ...
  async addMembersToChannel(channelId, workspaceId, memberList) {
    ...
    memberList.forEach(userId => {
      apiEventService.channelCreatedForUser(userId, channelId);
    });
