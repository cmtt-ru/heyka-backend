...
module.exports = Helpers.withDefaults({
  ...
  handler: async (request, h) => {
    ...
    try {
      const workspaceIds = await wdb.getWorkspaceIdsList();
      
      const mappedWorkspaceIds = workspaceIds.map(id => id.id);

      const countsChannelsInWorkspace = await Promise.all(mappedWorkspaceIds.map(async (id) => {

        const result = await chdb.getCountOfChannelsByWorkspaceId(id);
