...
exports.getSocketIO = async (server) => {
  ...
  io.on('connection', (socket) => {
    socket.uniqueName = getUniqueName(socket.id);
    console.log(`[${socket.id}, ${socket.uniqueName}]: Connected`);
    ...
    socket.on('error', (error) => {
      console.log(`[${socket.id}, ${socket.uniqueName}]: Connection error: ${error}`);
    });
  });
  ...
  server.apiEvents.on('subscribe-sockets', evt => {
    ...
    evt.sockets.forEach(socketId => {
      console.log(`Join ${socketId} in ${evt.room}`);
