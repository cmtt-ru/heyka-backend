...
module.exports = class PermissionService extends Schmervice.Service {
  ...
  /**
   * Can a user select a certain channel by limitOfChannelMembers
   * @param {string} channelId Channel id
   * @returns {boolean}
   */
  async canSelectChannelByLimit(channelId) {
    const { connectionService } = this.server.services();

    const channelConnections = await connectionService.getChannelConnections(channelId);

    return (channelConnections.length + 1 < config.limitOfChannelMembers);
  }
