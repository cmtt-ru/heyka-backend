...
module.exports = class DisplayService extends Schmervice.Service {
  ...
  user (user) {
    ...
    if (user.auth && user.auth.slack) {
      object.slack = {
        id: user.auth.slack.id,
      };
    }
    // check if user is guest
    if (!user.password_hash && !user.email && Object.keys(user.auth || {}).length === 0) {
      object.temporaryUser = true;
    }
    return object;
  }
  ...
  fullWorkspaceStateUser(user) {
    ...
    if (result.onlineStatus === 'sleep') {
      result.onlineStatus = 'idle';
    }
  ...
  /**
   * Returns secure object info of groups
   * @param {object} info Raw group object
   */
  group(info) {
    const result = {
      id: info.id,
      name: info.name,
      createdBy: info.creator_id,
      createdAt: info.created_at,
      updatedAt: info.updated_at,
    };
    return result;
