...
module.exports = class PermissionService extends Schmervice.Service {
  ...
  async canInviteChannel(channelId, userId) {
    ...
    return ['admin', 'moderator', 'user'].includes(channelRelation.role)
      && ['admin','super_admin', 'moderator', 'user'].includes(workspaceRelation.role);
  }
   ...
   * @param {string} userId 
   */
  async canViewInfoChannel(channelId, userId) {
    const {
      channelDatabaseService: chdb
    } = this.server.services();
   ...
   * @param {string} userId 
   */
  async canManageMembersChannel(channelId, userId) {
    const {
      channelDatabaseService: chdb
    } = this.server.services();
  ...
  async canUpdateWorkspace(workspaceId, userId) {
    ...
    return role === wdb.roles().admin || role === wdb.roles().super_admin || role === wdb.roles().moderator;
  }

  /**
   * Can user change role in workspace
   * @param {string} workspaceId Workspace id
   * @param {string} userId User id
   */
  async canChangeRoleInWorkspace(workspaceId, userId) {
    const {
      workspaceDatabaseService: wdb
    } = this.server.services();
    ...
    const role = relations[0].role;
    return role === wdb.roles().super_admin || wdb.roles().admin;
  }
  ...
  async canDeleteWorkspace(workspaceId, userId) {
    ...
    const role = relations[0].role;
    return role === wdb.roles().admin || role === wdb.roles().super_admin;
  }
  ...
  async canCreateChannelWorkspace(workspaceId, userId) {
    ...
    const role = relations[0].role;
    return role === wdb.roles().admin || role === wdb.roles().super_admin
      || role === wdb.roles().moderator || role === wdb.roles().user;
  }
  ...
  async canInviteWorkspace(workspaceId, userId) {
    ...
    if (workspace.can_users_invite) {
      return role === wdb.roles().admin || role === wdb.roles().moderator ||
        role === wdb.roles().user || role === wdb.roles().super_admin;
    } else {
      return role === wdb.roles().admin || role === wdb.roles().moderator || role === wdb.roles().super_admin;
    }
  }
  ...
  /**
   * Can create group in that workspace
   * 
   * @param {string} workspaceId 
   * @param {string} userId 
   * @returns {boolean}
   */
  async canCreateGroupWorkspace(workspaceId, userId) {
    const { 
      workspaceDatabaseService: wdb
    } = this.server.services();
   ...
   * @returns {boolean}
   */
  async canManageGroup(groupId, userId) {
    const { 
      workspaceDatabaseService: wdb,
    ...
    const relations = await wdb.getUserWorkspaceRelations(group.workspace_id, userId);
    if (relations.length === 0) {
      return false;
    } else if (relations.length > 1) {
      this.server.log(['warn'],
        `Multiple workspace_members relations: userId(${userId}), workspaceId(${group.workspace_id})`);
    }
  ...
  /**
   * Can view all workspace groups
   * 
   * @param {string} workspaceId 
   * @param {string} userId 
   * @returns {boolean}
   */
  async canViewWorkspaceGroups(workspaceId, userId) {
    const { 
      workspaceDatabaseService: wdb,
    } = this.server.services();
    ...
    if (relations.length === 0) {
      ...
      this.server.log(['warn'],
        `Multiple workspace_members relations: userId(${userId}), workspaceId(${workspaceId})`);
    }
   ...
   * @returns {boolean}
   */
  async canViewGroup(groupId, userId) {
    const { 
      workspaceDatabaseService: wdb,
      groupDatabaseService,
    } = this.server.services();
   ...
   * @returns {boolean}
   */
  async canUpdateGroup(groupId, userId) {
    const { 
      workspaceDatabaseService: wdb,
      groupDatabaseService,
    } = this.server.services();
  ...
  async canDeleteInvite(inviteId, userId) {
    ...
    return usrWsRelation.role === wdb.roles().admin || usrWsRelation.role === wdb.roles().super_admin ||
      usrWsRelation.role === wdb.roles().moderator;
  }
};
