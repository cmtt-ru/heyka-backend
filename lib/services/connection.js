...
module.exports = class ConnectionService extends Schmervice.Service {
  ...
  async disconnect(connectionId) {
    ...
    if (!connection) {
      ...
      if (Object.keys(user.auth || {}).length === 0 && !user.password_hash && !user.email) {
  ...
  async keepConnectionAlive(userId, connectionId, lifespan) {
    ...
    try {
      const onlineStatusLifespan = lifespan 
        || parseInt(process.env.ONLINE_STATUS_LIFESPAN || ONLINE_STATUS_LIFESPAN, 10);
      const connectionString = await client.get(redisKeys.connection(connectionId));
      if (!connectionString) return null;
      connectionObject = JSON.parse(connectionString);
  ...
  async renameConnection(prevConnectionId, newConnectionId, userId) {
    ...
    try {
      ...
      console.error(`[renameConnection] Error on unlock resource (${Date.now() - start}ms): `, e);
    }
