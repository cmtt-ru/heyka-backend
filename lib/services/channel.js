...
module.exports = class ChannelService extends Schmervice.Service {
  ...
  async selectChannel(channelId, userId, socketId, userMediaState) {
    ...
    const {
      oldChannelId,
      oldSocketId
    } = await chdb.switchUserToChannel(channelId, userId, socketId);

    if (oldChannelId === channelId && oldSocketId === socketId) {
      return;
    }

    // save user media state
    await udb.setUserMediaState(userId, userMediaState);

    // notify about that user unselected old channel
    if (oldChannelId) {
      apiEventService.userUnselectedChannel(userId, oldChannelId);
    }

    // notify about that user selected new channel
    apiEventService.userSelectedChannel(userId, channelId, userMediaState);

    // if there was oldSocketId, disable the old socket connection
    if (oldSocketId) {
      apiEventService.userChangedDevice(userId);
    }
  }
  ...
  async unselectChannel(channelId, userId, socketId) {
    ...
    const [
      ...
      chdb.getChannelByUserId(userId),
      chdb.getMainSocketForUser(userId)
    ]);

    // Канал не был выбран текущим пользователем
    if (currentUserChannel !== channelId) {
      throw new Error('ChannelIsNotSelected');
    }

    // Канал был выбран с другого девайса
    if (mainSocketForUser !== socketId) {
      throw new Error('ChannelIsSelectedOnAnotherDevice');
    }

    const usersInChannel = await chdb.removeUserFromChannel(userId);
  ...
  async updateUserMediaState(userId, socketId, userMediaState) {
    ...
    if (!channelId) throw new Error('User hasnt joined any channels');
