  ...
  handler: async (request, h) => {
    ...
    try {
      ...
      const codes = await Promise.all(
        emailList.map(email => workspaceService.inviteToWorkspace(workspaceId, userId, email))
      );
      ...
      await Promise.all(
        emailList.map((email, index) => emailService.sendInviteToWorkspace(email, workspace, user, codes[index]))
      );
