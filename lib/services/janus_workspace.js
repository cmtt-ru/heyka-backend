...
module.exports = class JanusWorkspaceService extends Schmervice.Service {
  ...
  async initJanusNodes() {
    ...
    if (!config.janus.k8sClusterHost) {
      ...
      // k8s cluster is used
      const k8sResult = await this.kubclient.api.v1.nodes.get({
        qs: {
          labelSelector: 'janus='
        }
      });

      console.log('K8S pods.get result: ', k8sResult);

      for (let i in k8sResult.items) {
        const janusNode = {
          ...defaultOpts
        };
        const addresses = k8sResult.items[i].metadata.status.addresses;
        for (let j in addresses) {
          switch (addresses[j].type) {
          case 'ExternalIP': janusNode.publicUrl = addresses[j].address; break;
          case 'InternalIP': janusNode.url = addresses[j].address; break;
