...
module.exports = class WorkspaceService extends Schmervice.Service {
  ...
  async getWorkspaceStateForUser(workspaceId, userId) {
    ...
    const usersForEachChannel = await Promise.all(
      channels.map(channel => channelService.usersMediaStateInChannel(channel.id))
    );
    channels.forEach((channel, index) => {
      channel.users = usersForEachChannel[index];
    });

    return { workspace, relation, channels, users };
  }
  ...
  async deleteChannel(channelId) {
    ...
    // collect all users of the channel before delete it
    const users = await chdb.getAllChannelMembers(channelId);
    ...
    this.server.apiEvents.emit(eventNames.server.channelDeleted, {
  ...
  async kickUserFromWorkspace(workspaceId, userId) {
    ...
    const allWorkspaceMembers = await wdb.getWorkspaceMembers(workspaceId);
    this.server.apiEvents.emit(eventNames.server.userLeavedWorkspace, {
      users: allWorkspaceMembers.map(u => u.id),
      userId,
      workspaceId
    });
  }
