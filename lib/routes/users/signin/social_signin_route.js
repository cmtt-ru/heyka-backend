...
module.exports = function createSocialSigninRoute({
  ...
  return [Helpers.withDefaults({
    ...
    handler: async function (request, h) {
      ...
      console.log("REDIS_URI >>>>>>>>>>>>", process.env.DATABASE_URL);
      ...
      try {
        ...
        if(service === 'facebook'){
          console.log(externalAvatarUrl);

          fileName = await axios({
            method: "get",
            url: externalAvatarUrl,
            responseType: "stream"
          }).then(function (response) {
            return  fileService.uploadS3(avatarFileId,response.data);
          });
        } else {
          console.log('else',externalAvatarUrl);
          fileName = await fileService.uploadS3FromUrl(externalAvatarUrl, avatarFileId);
        }
