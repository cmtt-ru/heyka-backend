...
module.exports = class UserService extends Schmervice.Service {
   ...
   * @param {boolean} emailVerified Is email already verified (user registered from email invite)
   * @returns {object} Ready to use user object
   */
  async signup (userInfo, emailVerified = false) {
    const {
      userDatabaseService,
      emailService,
    } = this.server.services();
    ...
    if (!userInfo.is_email_verified && userInfo.email && !userInfo.auth) {
  ...
  */
  ...
  /*
   * Preparing to kick User
   * @param {string} userId User id
   */
  async preparingToKickUser(userId){
    const {
      workspaceDatabaseService: wdb
    } = this.server.services();

    const userWorkspaceIds = await wdb.getWorkspaceIdsByUserIdWhereRoleSuperAdmin(userId);

    await Promise.all(userWorkspaceIds.map(async (id)=> {
      // delete a workspace if there are no Members in it
      const anotherMembers = await wdb.getMembersExceptUserId(id, userId);

      if (anotherMembers.length === 0) {
        await wdb.deleteWorkspace(id);
      }

      return true;
    } ));

    const userWorkspaceIdsAfterDeleted = await wdb.getWorkspaceIdsByUserIdWhereRoleSuperAdmin(userId);

    const isAdminInWorkspaces = await Promise.all(userWorkspaceIdsAfterDeleted.map(async (id) => {
      const roles = await wdb.getRolesByWorkspaceId(id);
    ...
    const workspaceIdsWhereIsNoAdmin = isAdminInWorkspaces.filter(w => w !== true);

    // Admins cannot delete account
    if (workspaceIdsWhereIsNoAdmin.length > 0) {
  ...
  async deleteAccount(userId, password) {
    ...
    await this.preparingToKickUser(user.id);

    const allUserWorkspaces = await wdb.getWorkspacesByUserId(user.id);

    // kick user from all workspaces
    await Promise.all(
      allUserWorkspaces.map(
  ...
  async refreshToken (
    ...
    const { userDatabaseService } = this.server.services();
    let tokenInfo = await userDatabaseService.findSession(refreshToken);
    let foundByPrev = false;

    // Refresh token is not found
    if (!tokenInfo) {
      // try to find session by previous refreshToken
      tokenInfo = await userDatabaseService.findSessionByPrevious(refreshToken);
    ...
    if (tokenInfo.access_token !== accessToken
      && tokenInfo.prev_access_token !== accessToken) throw new Error('AccessTokenNotMatched');
    ...
    // Delete access token by pre-previous access token (directly previous tokens should not be deleted)
    if (tokenInfo.prev_access_token && !foundByPrev) {
      await userDatabaseService.deleteAccessToken(tokenInfo.prev_access_token);
    }

    const user = await userDatabaseService.findById(tokenInfo.user_id);
    const newAccess = await this.createAccessToken(user, accessTokenLifespan);

    const refresh = uuid4();
    const updateRefreshToken = {
      refresh_token: refresh,
      access_token: newAccess,
      prev_refresh_token: foundByPrev ? tokenInfo.prev_refresh_token : tokenInfo.refresh_token,
      prev_access_token: foundByPrev ? tokenInfo.prev_access_token : tokenInfo.access_token,
  ...
  async sendInvite({
    ...
    // do not send invite to the same user
    ...
    // prevent send invites if user already in channel
    console.time(`sendInvites.getAllUserConnections-${toUserId}`);
    const userConnections = await connectionService.getUserConnections(toUserId, workspaceId);
    const connectionInChannel = userConnections.find(conn => conn.channelId === channelId);
    if (connectionInChannel) {
      return;
    }
    console.timeEnd(`sendInvites.getAllUserConnections-${toUserId}`);
  ...
  /**
   * Send invites between users using Slack
   * 
   * @param {string} fromUserId Sender user id
   * @param {string} toUserId Receiver user id
   * @param {string} workspaceId Workspace id
  ...
  async sendInviteBySlack({
    ...
    const [
      ...
      udb.findById(toUserId),
      udb.findById(fromUserId),
    ...
    if (!user.auth.slack) {
      throw new Error('UserNotConnectedSlack');
    }
    ...
    if (!workspace.slack.access_token) {
      throw new Error('WorkspaceNotConnectedSlack');
    }

    const inviteUrl = 'https://'
      + config.publicHostname
      + `/deeplink/channel-invite?channelId=${channel.id}&workspaceId=${workspace.id}`;
    await slackService.sendInviteToChannel(
      fromUser.name,
