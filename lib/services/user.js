...
module.exports = class UserService extends Schmervice.Service {
  ...
  */
  ...
  async deleteAccount(userId, password) {
    ...
    const userWorkspaceIds = await wdb.getWorkspaceIdsByUserIdWhereRoleSuperAdmin(user.id);
    ...
    await Promise.all(userWorkspaceIds.map(async (id)=> {
      ...
      if (anotherMembers.length === 0) {
        wdb.deleteWorkspace(id);
      }

      return true;
    } ));

    const userWorkspaceIdsAfterDeleted = await wdb.getWorkspaceIdsByUserIdWhereRoleSuperAdmin(user.id);
