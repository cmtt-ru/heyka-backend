...
module.exports = class ChannelService extends Schmervice.Service {
  ...
  async selectChannel(channelId, userId, socketId, userMediaState) {
    ...
    if (oldSocketId) {
      this.server.apiEvents.emit(eventNames.server.changedDevice, {
  ...
  async updateUserMediaState(userId, socketId, userMediaState) {
    ...
    this.server.apiEvents.emit(eventNames.server.mediaStateUpdated, {
      users: channelMembers.map(u => u.id),
      userId,
      userMediaState
    });
  }
