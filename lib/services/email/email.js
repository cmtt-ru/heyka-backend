...
// const i18n = new I18n({
//   locales: ['en', 'de'],
//   directory: path.join(__dirname, './../helpers/locales/email'),
//   defaultLocale: 'en',
//   updateFiles: false,
//   objectNotation: true,
// });
...
// /**
//  * configure shared i18n Singleton
//  */
//
// require('./../helpers/locales/email');
// i18n.configure({
...
module.exports = class EmailService extends Schmervice.Service {
  ...
  async sendWelcomeMail(user, token) {
    ...
    const data = {
      ...
      subject: i18n.__({ phrase: 'emails.welcomeLetter.subject', locale: user.lang || 'en' }),
      html
    };
  ...
  async sendEmailVerificationCode(user, token) {
    ...
    const data = {
      from: `Heyka Bot <bot@${config.credentials.mailgun.domain}>`,
      to: `${user.name} <${user.email}>`,
      subject: i18n.__({ phrase: 'emails.emailVerification.subject', locale: user.lang || 'en' }),
      html
    };
  ...
  async sendInviteToWorkspace(email, workspace, user, code) {
    ...
    const data = {
      from: `Heyka Bot <bot@${config.credentials.mailgun.domain}>`,
      to: email,
      subject: `${workspace.name} ${i18n.__({ phrase: 'emails.inviteToWorkspace.subject', locale: user.lang||'en' })}`,
      html
    };
  ...
  async sendResetPasswordToken(user, token) {
    ...
    const data = {
      from: `Heyka Bot <bot@${config.credentials.mailgun.domain}>`,
      to: `${user.name} <${user.email}>`,
      subject: i18n.__({ phrase: 'emails.resetPassword.subject', locale: user.lang || 'en' }),
      html
    };
