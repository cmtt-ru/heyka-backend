...
module.exports = class ChannelDatabaseService extends Schmervice.Service {
   ...
   */
  async switchUserToChannel(newChannelId, userId) {
    const { client } = this.server.redis;
    const oldChannelId = await client.get(`channel-for-user-${userId}`);
    if (oldChannelId) {
      await this.removeUserFromChannel(userId);
    }
    await client.set(`channel-for-user-${userId}`, newChannelId);
    await client.sadd(`users-in-channel-${newChannelId}`, userId);
    return oldChannelId;
  }
  ...
  async removeUserFromChannel(userId) {
    const { client } = this.server.redis;
    const channelId = await client.get(`channel-for-user-${userId}`);
    if (!channelId) return;
    await client.del(`channel-for-user-${userId}`);
    const users = await client.smembers(`users-in-channel-${channelId}`);
    if (!users || users.length === 0) return;
    const newList = users.filter(id => id !== userId);
    await client.del(`users-in-${channelId}`);
    await client.sadd(`users-on-${channelId}`, newList);
  }
  ...
  async getAllUsersInChannel(channelId) {
    const { client } = this.server.redis;
    return await client.smembers(`users-in-channel-${channelId}`);
  }
  ...
  async getChannelByUserId(userId) {
    const { client } = this.server.redis;
    return await client.get(`channel-for-user-${userId}`);
  }
};
