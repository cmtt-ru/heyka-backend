...
module.exports = class InviteCodesDatabaseService extends Schmervice.Service {
  ...
  /**
   * Return only actual channel invites
   * @param {string} channelId Channel id
   * @param {string} type Invite type
   */
  async getActualInvitesByChannel(channelId, type = null) {
    const now = new Date();
    const db = this.server.plugins['hapi-pg-promise'].db;
    if (type) {
      return db.any('SELECT * FROM invites WHERE channel_id=$1 AND type=$2 AND expired_at > $3', [
        channelId,
        type,
        now
      ]);
    } else {
      return db.any('SELECT * FROM invites WHERE channel_id=$1 AND expired_at > $2', [channelId, now]);
    }
  }
  ...
  async insertInvite (info) {
    const db = this.server.plugins['hapi-pg-promise'].db;
    const query = pgPromise.helpers.insert(info, columnSet);
    await db.none(query);
  }

  /**
   * Updates invite in the database
   * @param {string} inviteId
   * @param {object} update Update object
   */
  async updateInvite(inviteId, update) {
    const db = this.server.plugins['hapi-pg-promise'].db;
    const query = pgPromise.helpers.update(update, null, 'invites')
      + pgPromise.as.format(' WHERE id=$1', [inviteId]);
    await db.none(query);
  }
