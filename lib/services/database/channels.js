...
module.exports = class ChannelDatabaseService extends Schmervice.Service {
  ...
  async switchUserToChannel(newChannelId, userId) {
    ...
    const oldChannelId = await client.get(redisKeys.channelForUser(userId));
    ...
    await client.sadd(redisKeys.usersInChannel(newChannelId), userId);
    await client.set(redisKeys.channelForUser(userId), newChannelId);
