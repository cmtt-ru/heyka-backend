      ...
      if (user.device_tokens && user.device_tokens.length > 0) {
        ...
        results.forEach((result, index) => {
          if (result.message.includes('Endpoint is disabled')) {
            itemsToDelete.push(user.device_tokens[index]);
          }
        });

        if (itemsToDelete.length > 0) {
          user.device_tokens = user.device_tokens.filter(token => !itemsToDelete.includes(token));
          console.log(`Обновляем список токенов у юзера ${userId}, теперь у него вот такой: ${user.device_tokens}`);
          await udb.update(userId, {
