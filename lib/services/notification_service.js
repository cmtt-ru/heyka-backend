...
// const i18n = new I18n({
//   locales: ['en', 'de'],
//   directory: path.join(__dirname, './helpers/locales/notification'),
//   defaultLocale: 'en',
//   updateFiles: false,
//   objectNotation: true,
// });
...
module.exports = class NotificationService extends Schmervice.Service {
  ...
  async sendPushNotificationToDevice(deviceToken, message) {
    ...
    let params;
    try {
      params = await this.formatSNSParams(deviceEndpoint, message);
    } catch(e) {
      if (e.message !== 'Ingore') {
        throw e;
      }
   ...
   */
  async formatSNSParams(token, message) {
    let params;
    ...
    if (token.includes('GCM')) {
      ...
      const {
        channelDatabaseService: cdb,
        userDatabaseService: udb,
        
      } = this.server.services();

      const {name: userName, lang} = await udb.findById(message.data.userId);

      const {name: channelName} = await cdb.getChannelById(message.data.channelId);

      const localHeader = i18n.__(
        { phrase: 'notifications.channelInvite.header', locale:lang  },
        {userName, channelName}
      );

      console.log('I18N >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>', localHeader );
      ...
      throw new Error('Unknown device endpoint');
    }
