...
const AUTH_LINK_LIFESPAN = 10 * 60 * 1000; // a ten minutes
...
module.exports = class UserService extends Schmervice.Service {
  ...
  async updateProfile(userId, info) {
    ...
    const usersForEachWorkspace = await Promise.all(
      userWorkspaces.map(workspace => wdb.getWorkspaceMembers(workspace.id))
    );
    const usersToNotify = {};
    for (let wI = 0; wI < usersForEachWorkspace.length; wI++) {
      for (let uI = 0; uI < usersForEachWorkspace[wI].length; uI++) {
        if (!usersToNotify[usersForEachWorkspace[wI][uI].id]) {
          usersToNotify[usersForEachWorkspace[wI][uI].id] = true;
        }
      }
    }
    this.server.apiEvents.emit(eventNames.server.userUpdated, {
      users: Object.keys(usersToNotify),
      user: user
    });
