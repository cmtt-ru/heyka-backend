...
const INVITE_CODE_LIFESPAN = 30 * 24 * 3600 * 1000; // 30 days;
...
module.exports = class ChannelService extends Schmervice.Service {
  ...
  async selectChannel(channelId, userId, socketId, userMediaState) {
    ...
    return {
      httpsUrl: janus.publicHttpsUrl,
      wssUrl: janus.publicWssUrl,
  ...
  async getInviteChannelToken(channelId, userId) {
    ...
    const alreadyCreatedTokens = await codeService.getInvitesByChannel(channelId, 'channelInvite');
