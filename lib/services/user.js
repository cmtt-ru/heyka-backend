...
module.exports = class UserService extends Schmervice.Service {
  ...
  async createTokens (user, accessTokenLifespan = ACCESS_TOKEN_LIFESPAN, refreshTokenLifespan = REFRESH_TOKEN_LIFESPAN) {
    ...
    const refresh = uuid4();
    const now = new Date().toUTCString();
    const expired_at = new Date(Date.now() + refreshTokenLifespan).toUTCString();
  ...
  async refreshToken (accessToken, refreshToken, accessTokenLifespan = ACCESS_TOKEN_LIFESPAN, refreshTokenLifespan = REFRESH_TOKEN_LIFESPAN) {
    ...
    if (tokenInfo.access_token !== accessToken) throw new Error('Invalid credentials for refresing token');
    ...
    const updateRefreshToken = {
      refresh_token: refresh,
      access_token: access,
      expired_at: new Date(Date.now() + refreshTokenLifespan).toUTCString(),
      refreshed_at: new Date().toUTCString()
    };
