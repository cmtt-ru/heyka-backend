...
module.exports = class EmailService extends Schmervice.Service {
  ...
  constructor (...args) {
    ...
    // Init mailgun
    this.mg = mailgun({
      username: !isTest ? config.credentials.mailgun.domain : 'empty',
      apiKey: !isTest ? config.credentials.mailgun.apikey : 'empty',
  ...
  async sendResetPasswordToken(user, token) {
    const html = await this.emailRenderer.render('reset-password', {
      ...
      resetUrl: this.server.info.uri + RESET_PASSWORD_PATH + `?token=${token}`,
    });
