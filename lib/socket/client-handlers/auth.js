...
module.exports = (socket, io) => {
  return async function authHandler (data) {
    ...
    // Token is found
      ...
      // link socket with workspace id in redis, but not await
      udb.setWorkspaceForSocket(socket.id, workspaceId);
      ...
      // Update onlineStatus of the given user
      const onlineStatus = data.onlineStatus || onlineStatuses.online;
      await udb.setOnlineStatus(userId, workspaceId, onlineStatus);
      io.to(eventNames.rooms.workspaceRoom(workspaceId))
        .emit(eventNames.socket.onlineStatusChanged, {
          userId,
          onlineStatus
        });
