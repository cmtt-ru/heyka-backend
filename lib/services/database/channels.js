...
module.exports = class ChannelDatabaseService extends Schmervice.Service {
  ...
  /**
   * Returns count of channels by workspace_id
   * @param {string} id WorkspaceId
   */
  async getCountOfChannelsByWorkspaceId(id) {
    const db = this.server.plugins['hapi-pg-promise'].db;

    return db.query('SELECT COUNT(DISTINCT id) FROM channels WHERE workspace_id=$1', [id]);
  }
