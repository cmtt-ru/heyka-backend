...
module.exports = class WorkspaceService extends Schmervice.Service {
  ...
  async getWorkspaceStateForMonitoring(workspaceId, userId) {
    ...
    workspaceState.channel = workspaceState.channels.map(c => ({
      users: c.users,
      id: c.id,
      name: c.name,
      janus: c.janus,
    }));
    Object.values(janusStats).forEach(sess => {
      Object.values(sess).forEach(handle => {
        delete handle.sdps;
        if (handle.streams) {
          handle.streams.forEach(stream => {
    ...
    workspaceState.channels.forEach(ch => {
      ch.users.forEach(u => {
        ...
        u.janusStats = sessionsByUsers[ch.janusDetails.publicHttpsUrl][u.userId].map(sessionHandles => {
          let name = 'Unknown';
          if (Object.keys(sessionHandles).length === 2
            && Object.values(sessionHandles).find(h => h.plugin === 'janus.plugin.audiobridge')
            && Object.values(sessionHandles).find(h => h.plugin === 'janus.plugin.videoroom')) {
            name = 'Main window';
          } else if (Object.keys(sessionHandles).length === 1
            && Object.values(sessionHandles)[0].plugin === 'janus.plugin.textroom') {
            name = 'Frame window';
          }

          return {
            id: Object.values(sessionHandles)[0].session_id,
            name,
  ...
  async initMonitoringLoop(workspaceId) {
    ...
    setInterval(async () => {
      if (processing) {
        console.warn('Getting full workspace state with janus stats got more than 1 second');
        return;
      }
      ...
      try {
        ...
        // delete old records
        await db.result(
          'DELETE FROM janus_stats WHERE created_at < $1',
