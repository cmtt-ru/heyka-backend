  ...
  handler: async (request, h) => {
    ...
    try {
      const actions = actionsString.split(',').map(act => {
        ...
        if (!request.query[`${entity}Id`] && !entitiesNotRequiredId.includes(entity)) {
          requestErrors[act] = `Query parameter "${entity}Id" is required`;
        }
        return {
          action: act,
          entityId: request.query[`${entity}Id`],
          additionalData: request.query.additionalData,
      ...
      const resultArray = await Promise.all(actions.map(async act => {
        return {
          action: act.action,
          result: await act.method(act.entityId, userId, act.additionalData)
        };
      }));
