...
module.exports = class WorkspaceDatabaseService extends Schmervice.Service {
  ...
  /**
   * Returns roles list by id Workspace id
   * @param {uuid} workspaceId Workspace id
   */
  async getRolesByWorkspaceId (workspaceId) {
    const db = this.server.plugins['hapi-pg-promise'].db;
    
    const roles = await db.query(`SELECT wm.role FROM workspaces_members wm WHERE wm.workspace_id =$1`, [workspaceId]);
  ...
  /**
   * Returns all workspaces that the user belongs to and role = super_admin
   * @param {uuid} userId User guid
   */
  async getWorkspaceIdsByUserIdWhereRoleSuperAdmin(userId) {
    const db = this.server.plugins['hapi-pg-promise'].db;
    const q = `
      SELECT wm.workspace_id FROM workspaces_members wm 
      WHERE wm.user_id = $1 AND wm.role = 'super_admin'
    `;

    const workspaceIds = await db.any(q, [userId]);

    return workspaceIds.map(w => w.workspace_id);
  }
  ...
  /**
   * Returns an array of members of the workspace expect the specified user
   * @param {string} workspaceId Workspace id
   * @param {string} userId User id
   */
  async getMembersExceptUserId(workspaceId, userId) {
    const db = this.server.plugins['hapi-pg-promise'].db;
    const q = `
      SELECT wm.user_id FROM workspaces_members wm 
      WHERE wm.workspace_id = $1
    `;

    const userIds = await db.any(q, [workspaceId]);

    const mappedUserIds = userIds.map(id => id.user_id);

    return mappedUserIds.filter(id => id !== userId);
  }
