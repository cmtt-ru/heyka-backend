  ...
  async initJanusNodes() {
    ...
    if (!config.janus.k8sClusterHost) {
      const janusNodeOpts = {
        ...
        publicWssUrl: `${config.janus.defaultPublicJanusUrl}:8188`,
        name: 'default',
        ...defaultOpts
      };
      
      if (config.janus.defaultJanusUrl.includes('https://')) {
        janusNodeOpts.apiPort = 8089;
        janusNodeOpts.adminPort = 7889;
        janusNodeOpts.publicHttpsUrl = `${config.janus.defaultPublicJanusUrl}:8089/janus`;
        janusNodeOpts.publicWssUrl = `${config.janus.defaultPublicJanusUrl}:8989`;
      }

      this.janusNodes.push(janusNodeOpts);
    } else {
  ...
  async getJanuStatsForServer(janusServerInfo) {
    ...
    await Promise.all(sessionsResponse.sessions.map(async session => {
      ...
      await Promise.all(handles.map(async handle => {
        ...
        sessionResult.handles.push(info);
      }));
