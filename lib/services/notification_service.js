...
module.exports = class NotificationService extends Schmervice.Service {
  ...
  async formatSNSParams(token, message) {
    ...
    if (token.includes('GCM')) {
      ...
      const {name: userName} = await udb.findById(message.data.userId);

      const {name: channelName} = await cdb.getChannelById(message.data.channelId);

      const str = `${userName} приглашает вас в ${channelName}`;

      console.log('IOS STR >>>>>>>>>>>',str);

      const payload = {
        default: 'Дефолтное',
        APNS: JSON.stringify({
          aps: {
            alert: `${userName} приглашает вас в ${channelName}`,
            sound: 'default',
