...
module.exports = class ChannelDatabaseService extends Schmervice.Service {
  ...
  async getChannelMemberIds(channelId) {
    ...
    const q = 'SELECT chm.user_id FROM channels_members chm WHERE chm.channel_id = $1';
    const memberIds = await db.query(q, [channelId]);

    return memberIds.map(id => id.user_id);
  }
