...
module.exports = class UserDatabaseService extends Schmervice.Service {
  ...
  async setUserState(userId, socket, state) {
    const { client } = this.server.redis;
    schemas.userState.validate(state);
    await client.set(redisKeys.stateForUser(userId, socket), JSON.stringify(state));
  }

  async getUserState(userId, socket) {
    const { client } = this.server.redis;
    const stateJSON = await client.get(redisKeys.stateForUser(userId, socket));
