...
module.exports = class UserService extends Schmervice.Service {
   ...
   */
  async sendMessage(fromUserId, toUserId, isResponseNeeded, message) {
    const {
    ...
    apiEventService.privateMessage(toUserId, fromUserId, messageInfo.id, isResponseNeeded, message);

    if (isResponseNeeded) {
      // save message to the database
      await messageDatabaseService.addMessage(messageInfo.id, messageInfo);
      ...
      setTimeout(async () => {
        ...
        // and delete message 
        if (m) {
          apiEventService.noMessageResponse(fromUserId, toUserId, messageInfo.id);
  ...
  /**
   * Send response for specific message
   * 
   * @param {string} messageId Message id for response
   * @param {object} response Response data
   */
  async responseMessage(messageId, response) {
    const {
      messageDatabaseService,
      apiEventService
    } = this.server.services();

    const message = await messageDatabaseService.getMessage(messageId);

    if (!message) {
      throw new Error('NotFound');
    }

    await messageDatabaseService.deleteMessage(messageId);

    apiEventService.messageResponse(message.fromUserId, message.toUserId, messageId, response);
  }
};
