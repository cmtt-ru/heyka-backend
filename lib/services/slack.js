...
const JOIN_BY_INVITE_PATH = '/auth/social/slack/web-login';
...
module.exports = class SlackService extends Schmervice.Service {
   ...
   */
  async sendInviteToChannel (fromUserName, token, toId, workspaceName, channelName, inviteUrl) {
    try {
      let createResponse = await this.slack.conversations.open({
        token,
        users: toId
      ...
      await this.slack.chat.postMessage({
        ...
        channel: channelId,
        text: `*${fromUserName}* invited you `
        + `to *${channelName}* in *${workspaceName} in Heyka`,
        blocks: [
          {
            type: 'section',
            text: {
