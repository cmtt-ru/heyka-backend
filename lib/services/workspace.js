...
module.exports = class WorkspaceService extends Schmervice.Service {
  ...
  async addMembersToChannel(channelId, workspaceId, memberList) {
    ...
    const userIdsInChannel = await chdb.getChannelMemberIds(channelId);

    const filteredMembersList = memberList.filter(id => !userIdsInChannel.includes(id));

    if(filteredMembersList.length === 0){
      return ;
    }

    const countOfMembers = userIdsInChannel.length + filteredMembersList.length;

    if(countOfMembers > config.limitOfChannelMembers){
      throw new Error('Too many members in the channel');
    }
    ...
    const attachedUsers = workspaceMembers.filter(user => filteredMembersList.includes(user.id));
    ...
    if (!relations.length){
      throw new Error('There are no members in the workspace');
    }
    ...
    filteredMembersList.forEach(userId => {
    ...
    const connections = await Promise.all(filteredMembersList.map(userId => {
   ...
   */
  async inviteToWorkspace(workspaceId, userId, email = null) {
    const {
      inviteCodesDatabaseService: codeService
    } = this.server.services();
    const now = new Date();
  ...
  /**
   * Creates invite to the workspace by email
   * Only user with that email can join workspace by this invite
   * @param {string} workspaceId Workspace id
   * @param {string} userId User id
   * @param {string} email
   * @returns {object} fullCode and code info
   */
  async inviteToWorkspaceByEmail(workspaceId, userId, email) {
    const code = await this.inviteToWorkspace(workspaceId, userId, email);
    return code;
  }
