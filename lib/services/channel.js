...
module.exports = class ChannelService extends Schmervice.Service {
  ...
  async selectChannel(channelId, userId, socketId, userMediaState) {
    ...
    if (!janus) {
      janus = await janusWorkspaceService.prepareJanusChannel();
      janus.channelSecret = await helpers.getRandomCode(50);
      await chdb.setJanusForChannel(channelId, janus);
      // create janus channel
      await janusWorkspaceService.createAudioVideoRooms(channelId, janus);
    }
    ...
    await Promise.all([
      janusWorkspaceService.manageAuthTokensForChannel('add', [ channelAuthToken ], janus),
    ...
    if (conn.channelId) {
      await this.unselectChannel(channelId, userId, socketId);
    }
    ...
    return {
      url: janus.publicUrl,
      audioRoomId: janus.audioRoomId,
      videoRoomId: janus.videoRoomId,
      serverAuthToken,
      channelAuthToken
    };
  }
    ...
    if (channelConnections.length === 0) {
      ...
      if (channel.tmp_active_until) {
        ...
        if (now > activeUntil) {
          ...
          await Promise.all([
            janusWorkspaceService.deleteAudioVideoRooms(janusOpts),
            chdb.deleteJanusForChannel(channelId)
          ]);
        }
      } else {
      ...
      await Promise.all([
        ...
        janusWorkspaceService.manageAuthTokensForChannel('remove', [ channelAuthToken ], janusOpts)
      ]);
    }
  }
