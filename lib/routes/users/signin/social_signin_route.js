...
module.exports = function createSocialSigninRoute({
  ...
  return [Helpers.withDefaults({
    ...
    handler: async function (request, h) {
      ...
      // try {
      //   const imageUrl = await fileService.uploadImageFromUrl(externalAvatarUrl);
      // } catch(e) {
      ...
      try {
        ...
        const avatar = await fileService.uploadImageFromUrl(externalAvatarUrl);
      
        // Запрос не авторизован, просто создаём новый аккаунт
        if (!user && !authedUserId) {
          user = await userService.signup({
            name: externalName,
            // avatar, нужно ли сюда прокидывать, как я понял при каждом входе с другого соц ресурса ава будет меняться
            auth: {
              [service]: {
                id: externalId,
