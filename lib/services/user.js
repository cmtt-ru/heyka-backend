...
module.exports = class UserService extends Schmervice.Service {
  ...
  async signup (userInfo) {
    ...
    if (user.email) {
      const grUrl = helpers.getGravatarUrl(user.email);
      const s3Id = uuid4();
      user.avatar_set = fileService.getImgproxyImageSet('avatar', s3Id);
      parallelProcesses.push(fileService.uploadS3FromUrl(grUrl, s3Id));
    }
  ...
  */
  ...
  async updateProfile(userId, info) {
    ...
    if (info.avatarFileId) {
      const file = await fdb.getFileById(info.avatarFileId);
      console.log(file);
      if (!file) {
        ...
        throw new Error('FileIsNotAvatar');
      }
      info.avatar_file_id = info.avatarFileId;
      info.avatar_set = fileService.getImgproxyImageSet('avatar', file.filename);
