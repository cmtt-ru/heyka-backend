...
module.exports = class FileService extends Schmervice.Service {
  ...
  /**
   * Upload file to S3 And Insert In Db
   * @param {user} user
   * @param {string} externalAvatarUrl
   * @param {string} authedUserId
   * @returns {Promise<object>} fileDbInfo
   */
  async uploadFileToS3AndInsertInDb (user, externalAvatarUrl, authedUserId) {

    const {
      fileDatabaseService: fdb,
    } = this.server.services();

    const avatarFileIdForS3 = uuid();

    const avatarFileId = await this.uploadS3FromUrl(externalAvatarUrl, avatarFileIdForS3);

    const now  = new Date();
    const userId = user? user.id : authedUserId;

    const fileDbInfo = {
      id: uuid(),
      filename: avatarFileId,
      user_id: userId,
      created_at: now,
      type: 'avatar',
      updated_at: now
  ...
  uploadS3(id, fileStream) {
    return new Promise((resolve, reject) => {
      ...
      fileStream.on('end', async () => {
        /**
          * Stream end does not mean s3-file availability.
          * If we request file too soon, it might not be publically available yet,
          * and we'll get 404. This timeout is a small hack
          * so that we don't get 404 on big files.
          */
