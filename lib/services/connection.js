...
module.exports = class ConnectionService extends Schmervice.Service {
  ...
  async connect(
    ...
    //console.log(`User ${user.name}, ${user.email} tries to connect with ${onlineStatus}`);
    if (onlineStatus === 'sleep') {
      onlineStatus = 'online';
    }
  ...
  async disconnect(connectionId) {
    ...
    //console.log(`[${connectionId}]: Locked`);
    const start = Date.now();
    ...
    if (Object.keys(user.auth).length === 0 && user.invite_id && !user.email) {
   ...
   * @returns {object} Connection object
   */
  async keepConnectionAlive(userId, connectionId, lifespan) {
    const { client } = this.server.redis;
    const redlock = this.server.plugins['hapi-redlock-plugin'].redlock;
    const lock = await redlock.lock(`user-connections-${userId}`, 500);
   ...
   * @param {string} userId
   */
  async renameConnection(prevConnectionId, newConnectionId, userId) {
    const redlock = this.server.plugins['hapi-redlock-plugin'].redlock;
    const lock = await redlock.lock(`user-connections-${userId}`, 500);
    const start = Date.now();

    //(`[${newConnectionId}]: start to get prev connection object`);
    const conn = await this.getConnection(prevConnectionId);
    //console.log(`[${newConnectionId}]: finish to get prev connection object`, conn ? conn.channelId : 'undefined');
    //console.log(`[${prevConnectionId}, ${newConnectionId}]: Try to rename sessions ${!!conn}`);
    if (!conn) {
      await lock.unlock();
      return null;
    }
    ...
    //console.log(`[${newConnectionId}]: Start to set new connection objects`);
    await Promise.all([
